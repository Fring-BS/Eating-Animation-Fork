...
> **Unofficial fork** of the beloved [Eating Animation](https://modrinth.com/mod/eating-animation) mod by `theone_ss`, ported to NeoForge for Minecraft **26.3** using the new 1.21.4+ item model definition system. Original mod is MIT licensed; this fork is MIT licensed too. Full credit to the original author.
...
- **One jar, wide version support** — a single NeoForge jar covers Minecraft **26.3**.

---

## Installation

1. Install [NeoForge](https://neoforged.net/) for your Minecraft version (26.3)
...
- `neoforge.mods.toml` declares `minecraft` version range `[26.3,26.3.1)`. The item model definition format used by the assets is identical across that whole range, so one jar serves every version.
...
A: Make sure you downloaded the jar that matches your loader (Fabric or NeoForge) and that your Minecraft version is 26.3. The Fabric and NeoForge jars are NOT interchangeable — check the filename before installing.
